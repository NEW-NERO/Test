...
const unoDatabasePath = './database/uno_games.json';
...
await Rifky.relayMessage(m.chat, msg.message, {
...
`);
        ...
        case "hand":
            ...
            await Rifky.sendMessage(m.sender, { text: iniHandText }, { quoted: m });
        ...
        case "card":
            ...
            await Rifky.sendMessage(m.sender, { image: { url: cardImageUrl }, caption: cardText }, { quoted: m });
...
async function sendGameStatus(chat) {
    ...
    await Rifky.sendMessage(chat, { text: iniGameStatusText });
    await Rifky.sendMessage(chat, { image: { url: currentCardImageUrl }, caption: `Current Card: ${game.currentCard.color} ${game.currentCard.value}` });
}
}
break
case "giveaway": {
    ...
    if (subCommand === "start") {
        ...
        if (!LorenzoTheCreator) {
            ...
            if (users.length === 0) {
                ...
                const initialMessage = await Rifky.sendMessage(
                    m.chat,
...
break
case 'war': {
  Rifky.war = Rifky.war ? Rifky.war : {}
  Rifky.war2 = Rifky.war2 ? Rifky.war2 : {}
  // fungsi delay
  function sleep(ms) {
...
function getRandom(minn,maxx){
 ...
 let max = Math.floor(maxx)
  ...
  async function cekAFK(x){
    let turn = x
    let time = Rifky.war2[m.chat].time
    await sleep(90000)
    let turnNow = Rifky.war2[m.chat].turn
    let timeNow = Rifky.war2[m.chat].time
    // m.reply("Turn : " + turn + "-" + turnNow + "\n\nTime : " + time + "-" + timeNow)
    if (turn == turnNow && time == timeNow){
      Rifky.war[m.chat][turn].hp -= 2500
      sendButton(m.chat, `*@${Rifky.war[m.chat][turn].user.split('@')[0]} sedang AFK (Denda -2500 HP)*\n\n.war player = statistik pemain\n.attack @tag = serang lawan`)
      await sleep(3000)
      // cek kalau mati
      if (Rifky.war[m.chat][turn].hp <= 0) {
        sendButton(m.chat, `*@${Rifky.war[m.chat][turn].user.split('@')[0]} sudah mati karena HP (Health Point) habis.*`)
        // cek tim nya
        let playerTotal = 0
        let playerKalah = 0
        if (turn < 5){
          for (let i=0;i<5;i++){
            if (Rifky.war[m.chat][i].user != ""){
              playerTotal += 1
              if (Rifky.war[m.chat][i].hp <= 0)
              playerKalah += 1
            }
          }
          ...
          if (playerTotal > 0 && playerTotal == playerKalah){
            ...
            var teamAB = []
            for (let j=0;j<5;j++){
              if (Rifky.war[m.chat][j].user != ""){
                teamA.push(Rifky.war[m.chat][j].user)
                teamAB.push(Rifky.war[m.chat][j].user)
              }
            }
            for (let j=5;j<10;j++){
              if (Rifky.war[m.chat][j].user != ""){
                teamB.push(Rifky.war[m.chat][j].user)
                teamAB.push(Rifky.war[m.chat][j].user)
              }
            }
            sendButton(m.chat,  `*TEAM B MENANG KARENA TEAM A GOBLOK SEMUA*`)
            delete Rifky.war[m.chat]
            delete Rifky.war2[m.chat]
          }
        }else {
          for (let i=5;i<10;i++){
            if (Rifky.war[m.chat][i].user != ""){
          ...
          if (playerTotal == playerKalah){
            ...
            sendButton(m.chat,  `*TEAM A MENANG KARENA TEAM B GOBLOK SEMUA*`)
            delete Rifky.war[m.chat]
            delete Rifky.war2[m.chat]
          }
        }
      ...
      if (turn < 5){
        for (let i=5;i<10;i++){
          if (Rifky.war[m.chat][i].hp > 0 && Rifky.war[m.chat][i].user != "" && Rifky.war[m.chat][i].turn == false){
            Rifky.war2[m.chat].turn = i
            Rifky.war2[m.chat].time = +1
            pergantian = true
          }
        }
      }else {
      ...
      if (pergantian == false){
        for (let l=9;l>=0;l--){
          if (Rifky.war[m.chat][l].user != "" && Rifky.war[m.chat][l].hp > 0) {
            Rifky.war2[m.chat].turn = l
            Rifky.war2[m.chat].time = +1
          }
          Rifky.war[m.chat][l].turn == false
        }
      }
      await sleep(3000)
      sendButton(m.chat, `*Giliran @${Rifky.war[m.chat][Rifky.war2[m.chat].turn].user.split('@')[0]} untuk menyerang (Waktu 90 detik)*\n\n.war player = statistik pemain\n.attack @tag = serang lawan`)
      cekAFK(Rifky.war2[m.chat].turn)
    }
  }

  if (!(m.chat in Rifky.war)) return m.reply(`*Tidak ada game di grup ini.*`)
  if (!Rifky.war2[m.chat].war) return m.reply(`*War belom dimulai, ketik ".war start" untuk memulai pertarungan.*`)
  for (let i=0;i<10;i++){
    if (m.sender == Rifky.war[m.chat][i].user){
      if (i != Rifky.war2[m.chat].turn) {
        sendButton(m.chat, `*Sekarang adalah giliran @${Rifky.war[m.chat][Rifky.war2[m.chat].turn].user.split('@')[0]} untuk menyerang.*`)
        cekAFK(Rifky.war2[m.chat].turn)
      }
    }
  }
  ...
  if (Rifky.war2[m.chat].turn < 5){
    // return m.reply(args[0])
    for (let i=5;i<10;i++){
      if (Rifky.war[m.chat][i].user == args[0] && Rifky.war[m.chat][i].hp > 0){
       ...
       let  target = args[0]
        ...
        for (let i=0;i<10;i++){
          if (Rifky.war[m.chat][i].user == target){
            Rifky.war[m.chat][i].hp -= pointAttacker * 500
            Rifky.war[m.chat][Rifky.war2[m.chat].turn].turn = true
            sendButton(m.chat, `*@${attacker.split('@')[0]} menyerang @${target.split('@')[0]} sampai nyawanya berkurang ${pointAttacker * 500} (Sisa HP: ${Rifky.war[m.chat][i].hp})*\n\n*@${attacker.split('@')[0]} [${pointAttacker*10}%] - [${pointTarget*10}%] @${target.split('@')[0]}*\n*Level sangat mempengaruhi keberhasilan.*`)
            await sleep(2000)
            if (Rifky.war[m.chat][i].hp <= 0) sendButton(m.chat, `*@${target.split(`@`)[0]} sudah mati dalam pertarungan.*`)
            success = true
          }
        }
      }
    ...
    for (let i=0;i<5;i++){
      if (Rifky.war[m.chat][i].user == args[0] && Rifky.war[m.chat][i].hp > 0){
        ...
        for (let i=0;i<10;i++){
          if (Rifky.war[m.chat][i].user == target){
            Rifky.war[m.chat][i].hp -= pointAttacker * 500
            sendButton(m.chat, Rifky.war[m.chat][Rifky.war2[m.chat].turn].turn)
            Rifky.war[m.chat][Rifky.war2[m.chat].turn].turn = true
            sendButton(m.chat, Rifky.war[m.chat][Rifky.war2[m.chat].turn].turn)
            sendButton(m.chat, `*@${attacker.split('@')[0]} menyerang @${target.split('@')[0]} sampai nyawanya berkurang ${pointAttacker * 500} (Sisa HP: ${Rifky.war[m.chat][i].hp})*\n\n*@${attacker.split('@')[0]} [${pointAttacker*10}%] - [${pointTarget*10}%] @${target.split('@')[0]}*\n*Level sangat mempengaruhi keberhasilan.*`)
            await sleep(2000)
  ...
  if (Rifky.war2[m.chat].turn < 5){
    ...
    let turn1 = Rifky.war2[m.chat].turn
    let turn2 = Rifky.war2[m.chat].turn
    for (let k=5;k<10;k++){
      if (Rifky.war[m.chat][k].hp > 0 && Rifky.war[m.chat][k].user != "" && Rifky.war[m.chat][k].turn == false) {
        Rifky.war2[m.chat].turn = k
        Rifky.war2[m.chat].time = +1
        turn2 = Rifky.war2[m.chat].turn
      }
    }
    if (turn1 == turn2){
      for (i=0;i<10;i++){
        Rifky.war[m.chat][i].turn = false
      }
      for(i=0;i<5;i++){
        if (Rifky.war[m.chat][i].hp > 0 && Rifky.war[m.chat][i].user != "" && Rifky.war[m.chat][i].turn == false) {
          Rifky.war2[m.chat].turn = i
  ...
  let totalTurn = 0
  for (let i=0;i<10;i++){
    if (Rifky.war[m.chat][i].user != "") totalUser += 1
    if (Rifky.war[m.chat][i].turn == true) totalTurn += 1
  }
...
m.reply(textl);
  ...
  case 'audio':
    ...
    case 'image':
                    ...
                    case 'video':
const fileBufferr = await m.quoted.download()
                        const tempFilePathp = path.join(__dirname, `temp_image_${Date.now()}.mp4`);
                        fs.writeFileSync(tempFilePathp, fileBufferr);
if (/video/.test(mime)) {
                            ...
                            const hasilnya = await modepl.generateContent([
                                {
                                    fileData: {
...
.gemini2 image - chat menggunakan gambar/foto dengan gemini
.gemini2 audio - chat menggunakan audio dengan gemini
.gemini2 video - chat menggunakan audio dengan gemini
        `)
        break;
default:
Rifky.sendMessage(m.chat, { text: "Maaf kak, jika ada bantuan nya ketik .gemini2 help" });
}
}
break
case 'disk': {
...
const templateBlackjackMessage = (prefix, command, Rifky, m, blackjack) => {
...
`;
            global.db.data.users[Rifky.blackjack[m.chat].idPemain]
            delete Rifky.blackjack[m.chat];
            break;
        default:
            message = `*\`🃏 • B L A C K J A C K •\`*
...
Type *\`${prefix + command} stand\`* to end your turn.`;
    ...
    Rifky.blackjack = Rifky.blackjack || {};
    let aksi = args[0]
    let argumen = args[1]
    ...
    try {
        switch (aksi) {
            case 'end':
                if (Rifky.blackjack[m.chat]?.idPemain === m.sender) {
                    delete Rifky.blackjack[m.chat];
            ...
            case 'start':
                if (Rifky.blackjack[m.chat]) {
                    await reply(`*Sesi blackjack sudah berlangsung.* Gunakan *${prefix + command} end* untuk keluar dari sesi.`);
                } else {
                    Rifky.blackjack[m.chat] = new Blackjack(1);
                    Rifky.blackjack[m.chat].idPemain = m.sender;
                    let betAmount = argumen ? parseInt(argumen) : 1000;
                    Rifky.blackjack[m.chat].placeBet(betAmount);
                    Rifky.blackjack[m.chat].start();
                    const table = Rifky.blackjack[m.chat];
                    const pesanStart = templateBlackjackMessage(prefix, command, Rifky, m, table);
                    await reply(pesanStart);
                }
                break;

            case 'hit':
                if (!Rifky.blackjack[m.chat] || Rifky.blackjack[m.chat]?.idPemain !== m.sender) {
                    await reply('*Anda tidak sedang bermain blackjack atau bukan pemainnya.*');
                    break;
                }
                Rifky.blackjack[m.chat].hit();
                const tableHit = Rifky.blackjack[m.chat];
                const pesanHit = templateBlackjackMessage(prefix, command, Rifky, m, tableHit);
            ...
            case 'stand':
                ...
                Rifky.blackjack[m.chat].stand();
                const tableStand = Rifky.blackjack[m.chat];
                const pesanStand = templateBlackjackMessage(prefix, command, Rifky, m, tableStand);
            ...
            case 'double':
                ...
                Rifky.blackjack[m.chat].doubleDown();
                const tableDouble = Rifky.blackjack[m.chat];
                const pesanDouble = templateBlackjackMessage(prefix, command, Rifky, m, tableDouble);
...
const detectZero = (map, x, y) => {
    ...
    Rifky.minessweeper = Rifky.minessweeper || {};
    const orgs = args[0];
    const oX = args[1];
    const oY = args[2];
    const F = args[3];
    ...
    switch (orgs.toLowerCase()) {
        ...
        case "stop":
        case "end":
            if (!Rifky.minessweeper[m.chat]) {
                return reply('🚨 *No active game session.*');
            }
            delete Rifky.minessweeper[m.chat];
        ...
        case "buka":
            ...
            const g = Rifky.minessweeper[m.chat];
            ...
            Rifky.minessweeper[m.chat].key = newKey;
            return;
    }
};
break
case 'ulartangga': {
  class SnakeAndLadderGame {
    ...
    startGame = async (m, player1Name, player2Name) => {
        ...
        const {
            key
        } = await Rifky.sendMessage(m.chat,{image: await this.getBoardBuffer()});
        this.keyId = key;
    };
    ...
    playTurn = async (m, player) => {
        ...
        await Rifky.sendMessage(m.chat, {image: {url: "https://i.pinimg.com/originals/2f/68/a7/2f68a7e1eee18556b055418f7305b3c0.jpg" }, caption: `🎲 ${this.formatPlayerName(player)} *melempar dadu.*\n\n  - Dadu: *${diceRoll}*\n  - Dari kotak: *${this.currentPositions[player]}*\n  - Ke kotak: *${this.currentPositions[player] + diceRoll}*` }, {quoted: m});
...
class GameSession {
    ...
    Rifky.ulartangga = Rifky.ulartangga || {};
    const sessions = (Rifky.ulartangga_ = Rifky.ulartangga_ || {});
    const sessionId = m.chat;
    const session = sessions[sessionId] || (sessions[sessionId] = new GameSession(sessionId, Rifky));
    const game = session.game;
    const {
        state
    } = Rifky.ulartangga[m.chat] || {
        state: false
    };
